		...
		success: function(data, status){
			...
			if (data.length == 0){
				...
				$.each(data, function(i,item){
					...
					var iDireccion = item.Direccion==null?'No disponible':item.Direccion;
					var iCodigoPostal = item.CodigoPostal==null?'':item.CodigoPostal;
					var iTelefono = item.Telefono==null?'':item.Telefono;
					var iCapacidadDepositos = item.CapacidadDepositos==null?'No disponible':item.CapacidadDepositos;
					var iBotellasStock = item.BotellasStock==null?'No disponible':item.BotellasStock;
					...
					f_winery_address.append(iDireccion + (iDirMapa != '' ? '    <a id="a_map_query" href="#"  class="ui-btn ui-btn-inline ui-icon-location ui-btn-icon-notext ui-corner-all ui-btn-b">Llamar</a>':''));
					...
					f_winery_phone_number.append(iTelefono + (iTelefono != '' ? '    <a id="a_phone" href="#"  class="ui-btn ui-btn-inline ui-icon-phone ui-btn-icon-notext ui-corner-all ui-btn-b">Llamar</a>':'No disponible'));
					...
					f_winery_email.append(iDirCorreo + (iDirCorreo != ''? '    <a id="a_email" href="#"  class="ui-btn ui-btn-inline ui-icon-mail ui-btn-icon-notext ui-corner-all ui-btn-b">Escribir</a>':'No disponible'));
					f_winery_webpage.append(iDirWeb + (iDirWeb != ''? '    <a id="a_web" href="#" rel="external" class="ui-btn ui-btn-inline ui-icon-eye ui-btn-icon-notext ui-corner-all ui-btn-b ">Visitar</a>':'No disponible'));
					
					//f_winery_comments.children('.ui-collapsible-content').append(iObservaciones);
					f_winery_comments.append(iObservaciones);
					...
					console.log("dirmapa" + iDirMapa);
					console.log("gupdirmapa: " + decodeURIComponent(gupMap(iDirMapa, "q")));
					
					/*
					GMaps.geocode({
						  address: decodeURIComponent(gupMap(iDirMapa, "q")),
					...
					*/
					...
					a_map_query.attr("onclick", "window.open('" + iDirMapa + "', '_system');" );
					// ANDROID -> a_map_query.attr("onclick", "navigator.app.loadUrl('" + iDirMapa + "', {openExternal:true});" );
					
					
					a_email.attr("href", "mailto:" + iDirCorreo);
					a_email.attr("rel", "external");					
					
					a_web.attr("onclick", "window.open('http://" + iDirWeb + "', '_system');" );
					// ANDROID -> a_web.attr("onclick", "navigator.app.loadUrl('http://" + iDirWeb + "', {openExternal:true});" );
...
//$(document).ready(function(){
	...
	$('#ui-id-1').addClass('ui-btn-active');
